...
For every supported poll, the router creates at most one bridge-owned editable live-result companion per poll endpoint, including the source endpoint. Companions show aggregate option counts only in the fixed bold/italic live-results layout documented in `README.md`; they never show voter names, HMAC IDs, per-voter choices, or imply that multi-select counts equal unique voters. The companion rows contain only lifecycle metadata, and edits and deletion use the existing per-endpoint delivery lanes, idempotency ledger, and mutation revisions. If a source contribution cannot be observed, the companion excludes it from totals rather than fabricating zero.
