...
The default child-context display mode is `opaque`. The explicit `friendly` mode may retain bounded normalized thread/topic labels in the presentation-only `child_scope_labels` table; switching back to `opaque` clears them. Labels never participate in routing, identity, or copy lookup.
