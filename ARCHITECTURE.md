...
- Account administration is centralized behind the `admin` role policy: `GET /api/users`, invite creation, activation changes, reset-token creation, and `GET /api/audit` are admin-only. Operators may use the existing operational APIs and later membership review APIs, but cannot administer accounts. Invite and reset tokens are one-time random bearer values stored only as hashes; audit rows contain fixed actions and opaque target IDs.
