...
Polls retain their native UI on destinations that support the source semantics. A destination that cannot represent the question, options, answer mode, duration, or media combination receives deterministic text instead of silently losing poll semantics. The bridge-owned live-result companion is aggregate-only and uses this exact layout: `📊` followed by the bold-italic `LIVE POLL RESULTS ACROSS ALL GROUPS`, `❓` plus the normal question, bold `Options`, then italic option labels with `○`, `—`, and `votes`; it never shows voter names or per-voter choices. It is updated through the normal delivery lanes. Deleting a poll also deletes its live-result companion on every endpoint. `aggregate-response` remains supported as an exact reply to a poll copy; the trigger message is suppressed and the reply produces the same live-results layout using canonical counts. Bot-created Telegram polls provide absolute result snapshots. Human-created Telegram source polls can be mirrored but have a partial/unavailable live source contribution because the Bot API does not expose the required ongoing voter/result lifecycle; MTProto is intentionally not used.
...
- **Membership fulfillment**: Approved requests use narrow WhatsApp/Discord administration boundaries. WhatsApp fulfillment never adds participants directly: it requires join approval, sends a secure invite, and leaves the request pending until the applicant's join request is safely matched and approved; provider failures expose only safe status classes.
- **WhatsApp membership readiness**: Administrators can inspect safe, connection-scoped join-approval readiness through `/api/connections/{id}/membership-readiness?endpointAlias=...`; it exposes only fixed readiness/failure classes.
- Approved WhatsApp applicants receive a short-lived opaque join URL through the existing verified-email mailer; only that approved URL can obtain the current invite, and it is revoked after fulfillment or request deletion.
- **Membership review**: Authenticated admins and operators can filter and review requests, inspect protected evidence, and make audited approve/reject/needs-review decisions. Concurrent stale decisions return a conflict; AI remains advisory. Evidence MIME is preserved for the optional analyzer, and local evidence is purged immediately after a final approve/reject decision; `needs_review` retains it.
- **Control-plane retention**: Expired sessions/challenges/invites and completed or terminal membership requests are pruned in bounded batches after the retention window. Final approve/reject clears local evidence immediately; startup/daily maintenance also removes old orphan evidence left by a crash or failed unlink while protecting evidence still referenced by an active request. Routing state is not affected.
...
Admins create verification pipelines in the Web UI or through `/api/verification/pipelines`, selecting a configured endpoint alias that belongs to a sync set and, for Discord, a role ID. Each sync set can define bounded applicant instructions, evidence instructions, reviewer guidance, and up to 32 `text`, `textarea`, `select`, or `checkbox` custom fields through `/api/sync-sets/{id}/membership`; the configuration is kept in `control.db`. The public pipeline link accepts the sync set's configured form, a work email, transport identity, optional LinkedIn URL, and bounded PDF/image evidence. Optional email delivery uses `VERIFICATION_MAIL_API_KEY` and `VERIFICATION_MAIL_FROM`; optional OpenRouter review additionally requires `OPENROUTER_ALLOW_TRAINING=false`. Operators and admins review email-verified requests in the Membership view. Decisions are authoritative, AI is advisory, and fulfillment retries are explicit/idempotent. Terminal request records are retained for 30 days before bounded cleanup, while raw local evidence is purged immediately on approve/reject and startup/daily maintenance removes stale orphan evidence. Set `VERIFICATION_PUBLIC_BASE_URL` when the externally reachable membership URL differs from the request host seen by message-sync (for example, behind a reverse proxy).
