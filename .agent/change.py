...
for n, line in enumerate(live.splitlines(), 1):
    if line.startswith('func ') or line.startswith('type ') or 470 <= n <= 900:
        print(f'MTPROTO_LIVE:{n:04d}:{line}')
for n, line in enumerate(test.splitlines(), 1):
    if line.startswith('func Test') or line.startswith('type fake'):
        print(f'MTPROTO_TEST:{n:04d}:{line}')
