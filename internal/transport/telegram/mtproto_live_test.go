...
type fakeMTLiveAuth struct {
	...
	lastKind          string
	lastText          string
	lastTextEntities  []tg.MessageEntityClass
	lastReplyID       int
	lastTopicID       int
	lastMedia         []byte
	lastMediaEntities []tg.MessageEntityClass

	reactionEmoji string
	editText      string
	editEntities  []tg.MessageEntityClass
	deletedID     int
	download      []byte
...
func (f *fakeMTLiveAuth) SendText(_ context.Context, peer mtprotoPeerState, text mtprotoFormattedText, replyID, topicID int) (int, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.lastPeer, f.lastKind, f.lastText, f.lastReplyID, f.lastTopicID = peer, "text", text.Text, replyID, topicID
	f.lastTextEntities = append([]tg.MessageEntityClass(nil), text.Entities...)
	if f.nextID == 0 {
		f.nextID = 100
	}
	return f.nextID, nil
}
func (f *fakeMTLiveAuth) SendMedia(_ context.Context, peer mtprotoPeerState, kind string, data []byte, text mtprotoFormattedText, replyID, topicID int) (int, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.lastPeer, f.lastKind, f.lastText, f.lastReplyID, f.lastTopicID = peer, kind, text.Text, replyID, topicID
	f.lastMedia = append([]byte(nil), data...)
	f.lastMediaEntities = append([]tg.MessageEntityClass(nil), text.Entities...)
	if f.nextID == 0 {
		f.nextID = 101
	}
	return f.nextID, nil
...
func (f *fakeMTLiveAuth) Edit(_ context.Context, peer mtprotoPeerState, _ int, text mtprotoFormattedText) error {
	f.mu.Lock()
	f.lastPeer, f.editText = peer, text.Text
	f.editEntities = append([]tg.MessageEntityClass(nil), text.Entities...)
	f.mu.Unlock()
	return nil
}
...
func TestMTProtoLiveFormatsForwardedTextAndLivePollResults(t *testing.T) {
	live := &fakeMTLiveAuth{groups: []mtprotoGroup{{
		Peer: mtprotoPeerState{RemoteID: "-1000000000042", Kind: "channel", ID: 42, AccessHash: 4242},
	}}}
	adapter, _ := newMTProtoLiveAdapter(t, "mt-format", "-1000000000042", live)
	defer adapter.Close()

	const result = "📊 ***LIVE POLL RESULTS ACROSS ALL GROUPS***\n❓ w1\n\n**Options**\n○ *q* — 1 votes\n○ *s* — 1 votes"
	ref, err := adapter.Send(context.Background(), transport.Outgoing{Endpoint: "tg", Kind: "text", RenderedText: result})
	if err != nil {
		t.Fatal(err)
	}
	const want = "📊 LIVE POLL RESULTS ACROSS ALL GROUPS\n❓ w1\n\nOptions\n○ q — 1 votes\n○ s — 1 votes"
	if live.lastText != want {
		t.Fatalf("MTProto formatted send = %q, want %q", live.lastText, want)
	}
	assertMTProtoFormattingEntities(t, live.lastTextEntities)
	if err := adapter.Edit(context.Background(), ref, result); err != nil {
		t.Fatal(err)
	}
	if live.editText != want {
		t.Fatalf("MTProto formatted edit = %q, want %q", live.editText, want)
	}
	assertMTProtoFormattingEntities(t, live.editEntities)
}

func assertMTProtoFormattingEntities(t *testing.T, entities []tg.MessageEntityClass) {
	t.Helper()
	var bold, italic int
	...
	if bold != 2 || italic != 3 {
		t.Fatalf("MTProto formatting entities = %#v; want 2 bold and 3 italic", entities)
	}
}
