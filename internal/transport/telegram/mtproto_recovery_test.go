...
func TestMTProtoRecoveryOrderingBoundsCursorAndTopics(t *testing.T) {
	...
	auth := &fakeMTRecoveryAuth{
		fakeMTLiveAuth: &fakeMTLiveAuth{
			groups: []mtprotoGroup{{Peer: mtprotoPeerState{RemoteID: "-1000000000042", Kind: "channel", ID: 42, AccessHash: 9}, Forum: true}},
			topics: []mtprotoTopic{{ID: 17, Title: "Planning"}},
		},
	...
	if len(got) != 2 || got[0].RemoteID != "2" || got[1].RemoteID != "3" {
		t.Fatalf("ordering/bounds: %+v", got)
	}
	if got[0].ChildScope == nil || got[0].ChildScope.RemoteID != "17" {
		t.Fatalf("topic lost: %+v", got[0].ChildScope)
	}
	if got[0].ChildScope.Label != "Planning" {
		t.Fatalf("recovered topic label = %q, want %q", got[0].ChildScope.Label, "Planning")
	}
	if got[0].Checkpoint.StreamKey != stream || got[0].Checkpoint.Position != 2 {
		t.Fatalf("checkpoint: %+v", got[0].Checkpoint)
	}
}
...
func TestMTProtoRecoveryAgeMediaLimitAndCancellation(t *testing.T) {
	...
	if len(got) != 1 || got[0].MediaLoader == nil {
		t.Fatalf("age/media: %+v", got)
	}
	if _, err := got[0].MediaLoader(context.Background()); !errors.Is(err, errMTProtoMediaTooLarge) {
		t.Fatalf("media limit err=%v", err)
	}
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if err := a.Recover(ctx, transport.RecoveryRequest{Cursor: transport.Checkpoint{StreamKey: stream, Valid: true}, MaxEvents: 1, MaxAge: time.Hour}, func(context.Context, transport.Incoming) error { return nil }); !errors.Is(err, context.Canceled) {
...
func TestMTProtoRecoveryStreamsAndLiveCheckpointAreConnectionScoped(t *testing.T) {
	...
	if as == bs {
		t.Fatal("connection streams collided")
	}
	a.normalizeMTProtoMessage(context.Background(), historyEntities(), histMsg(9, time.Now(), 0), false)
	select {
	case in := <-a.Events():
		if in.Checkpoint.StreamKey != as || in.Checkpoint.Position != 9 {
			t.Fatalf("live checkpoint %+v", in.Checkpoint)
		}
	case <-time.After(time.Second):
