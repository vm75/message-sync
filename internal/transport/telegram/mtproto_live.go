...
import (
	...
	gotdentity "github.com/gotd/td/telegram/message/entity"
	gotdhtml "github.com/gotd/td/telegram/message/html"
	gotdstyling "github.com/gotd/td/telegram/message/styling"
...
type mtprotoFormattedText struct {
	Text     string
...
type mtprotoLiveClient interface {
	...
	SendText(context.Context, mtprotoPeerState, mtprotoFormattedText, int, int) (int, error)
	SendMedia(context.Context, mtprotoPeerState, string, []byte, mtprotoFormattedText, int, int) (int, error)
	React(context.Context, mtprotoPeerState, int, string) error
	Edit(context.Context, mtprotoPeerState, int, mtprotoFormattedText) error
...
func (a *MTProtoAdapter) handleMTProtoUpdates(ctx context.Context, updates tg.UpdatesClass) error {
	...
	dispatcher.OnMessagePollVote(func(handlerCtx context.Context, _ tg.Entities, update *tg.UpdateMessagePollVote) error {
		a.handleMTProtoPollVoteUpdate(handlerCtx, update)
		return nil
	})
...
func (a *MTProtoAdapter) sendMTProto(ctx context.Context, outgoing transport.Outgoing) (transport.MessageRef, error) {
	...
	if kind == "poll" {
		...
		if supported {
			...
			if native {
				done := a.live.markPendingSend(outgoing.Endpoint, "poll", topicID, question.Text)
	...
	case "text":
		...
		formatted, formatErr := mtprotoPresentation(content)
		if formatErr != nil {
			return transport.MessageRef{}, formatErr
		}
		done := a.live.markPendingSend(outgoing.Endpoint, kind, topicID, formatted.Text)
		id, err = client.SendText(ctx, peer, formatted, replyID, topicID)
		done(err == nil)
	case "image", "video", "audio", "document", "sticker":
		...
		id, err = client.SendMedia(ctx, peer, kind, outgoing.MediaBytes, formatted, replyID, topicID)
		done(err == nil)
	default:
...
func (a *MTProtoAdapter) editMTProto(ctx context.Context, ref transport.MessageRef, text string) error {
	...
	formatted, err := mtprotoPresentation(text)
	if err != nil {
		return err
	}
	done := a.live.markPendingMutation("edit", ref.Endpoint, id)
	err = client.Edit(ctx, peer, id, formatted)
	done(err == nil)
	if err != nil {
...
func mtprotoPresentation(content string) (mtprotoFormattedText, error) {
	rendered, mode := telegramPresentation(content)
	if mode != telegrambotmodels.ParseModeHTML {
		return mtprotoFormattedText{Text: rendered}, nil
	}
	return mtprotoHTMLPresentation(rendered)
}

func mtprotoPollQuestion(outgoing transport.Outgoing) (mtprotoFormattedText, error) {
	question := strings.TrimSpace(outgoing.SourceText)
	if attribution := telegramPollAttribution(outgoing); attribution != "" {
		return mtprotoHTMLPresentation(attribution + " " + html.EscapeString(question))
	}
	return mtprotoFormattedText{Text: question}, nil
}

func mtprotoHTMLPresentation(rendered string) (mtprotoFormattedText, error) {
	var builder gotdentity.Builder
	if err := gotdstyling.Perform(&builder, gotdhtml.String(nil, rendered)); err != nil {
		return mtprotoFormattedText{}, errors.New("format Telegram MTProto message")
	}
	text, entities := builder.Complete()
	return mtprotoFormattedText{Text: text, Entities: entities}, nil
}
...
func (c *gotdAuthClient) SendText(ctx context.Context, peer mtprotoPeerState, text mtprotoFormattedText, replyID, topicID int) (int, error) {
	...
	updates, err := tg.NewClient(c.client).MessagesSendMessage(ctx, &tg.MessagesSendMessageRequest{
		Peer: input, ReplyTo: mtprotoReply(replyID, topicID), Message: text.Text, Entities: text.Entities, RandomID: randomID,
	})
...
func (c *gotdAuthClient) SendMedia(ctx context.Context, peer mtprotoPeerState, kind string, data []byte, caption mtprotoFormattedText, replyID, topicID int) (int, error) {
	input, err := peer.input()
	if err != nil {
		return 0, err
	}
	...
	updates, err := raw.MessagesSendMedia(ctx, &tg.MessagesSendMediaRequest{
		Peer: input, ReplyTo: mtprotoReply(replyID, topicID), Media: media, Message: caption.Text, Entities: caption.Entities, RandomID: randomID,
	})
	return gotdunpack.MessageID(updates, err)
}
...
func (c *gotdAuthClient) Edit(ctx context.Context, peer mtprotoPeerState, id int, text mtprotoFormattedText) error {
	input, err := peer.input()
	if err != nil {
		return err
	}
	_, err = tg.NewClient(c.client).MessagesEditMessage(ctx, &tg.MessagesEditMessageRequest{
		Peer: input, ID: id, Message: text.Text, Entities: text.Entities,
	})
	return err
}
