/**
 * Message Sync • Lightweight Zero-Dependency QR Code Generator
 * Supports QR Code Model 2 (versions 1-40, Byte Mode, Error Correction L/M/Q/H)
...
(function () {
  ...
  // QR Code generation algorithms (Kazuhiko Arase QR library algorithm simplified)
  ...
  const QRUtil = {
    ...
    getErrorCorrectionPolynomial(errorCorrectLength) {
  ...
  const QRCode = {
     ...
     * @param {string} text
     * @param {object} options { size: 256, margin: 4, lightColor: '#ffffff', darkColor: '#000000' }
     */
      ...
      for (let row = 0; row < count; row++) {
        for (let col = 0; col < count; col++) {
          if (qr.isDark(row, col)) {
            const x = Math.round((col + margin) * moduleSize);
            const y = Math.round((row + margin) * moduleSize);
            const w = Math.ceil((col + margin + 1) * moduleSize) - x;
            const h = Math.ceil((row + margin + 1) * moduleSize) - y;
