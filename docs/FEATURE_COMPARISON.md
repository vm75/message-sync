...
**Comparison snapshot:** 2026-09-02
...
| Discord thread/forum behavior | Incoming thread/forum messages are flattened to the configured parent endpoint; no dynamic child endpoint is persisted | Thread-oriented bridge targets and provisioning flows are available | Simplified to avoid dynamic routing identity. |
| Telegram forum topics | Incoming topic messages are flattened to the configured parent chat | Telegram forum-topic discovery and bridge configuration are available through MTProto | Simplified for Bot API operation and stable endpoint identity. |
...
| Sender presentation in Discord | One managed webhook per destination channel; transient sender display name is supplied as the webhook APP username, with HMAC fallback | Webhook delivery can override username/presentation per forwarded sender | Preserved and narrowed so sender presentation is transient metadata rather than persistent identity. |
