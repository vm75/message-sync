...
| Friendly child-context presentation | Optional global mode renders `group/user` or `group:topic-or-thread/user`; labels are local presentation metadata, generic when unknown, and never used for routing | Platform-specific presentation and topic metadata are available in bridge flows | Deliberately opt-in; opaque mode remains the privacy-first default and disabling friendly mode clears its label catalog. |
