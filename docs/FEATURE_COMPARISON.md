...
| WhatsApp membership fulfillment | Human-approved WhatsApp invite plus pending join-request matching; no server-side direct add or per-member invite rotation | Direct participant add and membership tooling are implemented | Narrowed to an approval-gated, identity-safe invite flow. |
