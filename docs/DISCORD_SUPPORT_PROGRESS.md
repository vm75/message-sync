...
| 2 | [#17](https://github.com/vm75/message-sync/issues/17) | Generalize REST API and sync-set CRUD for transport endpoints | #16 | In progress | Transport-neutral `/api/endpoints` CRUD, mixed-transport sync sets, WhatsApp-only `/api/groups` compatibility, safe logging, validation, tests, and docs implemented on `agent/discord-support`. Completion is blocked only on required manual `make fmt`, `make test`, and `make vet` verification; `VERSION` remains `0.1.0`. |
