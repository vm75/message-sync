...
| 2 | [#17](https://github.com/vm75/message-sync/issues/17) | Generalize REST API and sync-set CRUD for transport endpoints | #16 | Complete | Implemented transport-neutral `/api/endpoints` CRUD, mixed-transport sync-set membership, WhatsApp-only `/api/groups` compatibility, safe error logging, Discord target validation, tests, and docs. Manual verification at `3f2783a`: `make fmt` clean, `git diff --exit-code` clean, `make test` passed, `make vet` passed, and `VERSION` remained `0.1.0`. Next ticket can rely on configured Discord endpoint aliases and mixed sync sets, but runtime Discord ingestion remains out of scope here. |
| 3 | [#18](https://github.com/vm75/message-sync/issues/18) | Discord adapter foundation, bot lifecycle, channel event ingestion | #16, #17 | Complete | Implemented DiscordGo v0.29 gateway ingress adapter with configured-channel alias filtering, `discord:`-namespaced HMAC actor IDs, bridge-bot and bridge-managed-webhook loop prevention, safe classified logging, env/secret token loading, narrow `ChannelWebhook` boundary, app lifecycle wiring, docs, and unit tests. Manual verification at `07fd65b`: `make fmt` clean, `git diff --check` clean, `git diff --exit-code` clean, `make test` passed, `make vet` passed, `podman build` passed, `podman compose config` passed, and `VERSION` remained `0.1.0`. |
| 4 | [#19](https://github.com/vm75/message-sync/issues/19) | Wire multiple adapters into application/router | #16, #18 | Complete | Implemented alias→transport adapter registry, one ordered WhatsApp/Discord ingress loop, destination transport dispatch, mixed fan-out tests, and restart-safe partial-fan-out coverage. Implementation commits `bd2d1c1` + `7d81d84`; manual verification at `5728597`: `make fmt` clean, `git diff --check` clean, `git diff --exit-code` clean, `make test` passed, `make vet` passed, `git diff --exit-code VERSION` passed, and `VERSION` remained `0.1.0`. Next ticket can rely on transport-aware dispatch; Discord outbound lifecycle methods remain intentionally unimplemented until #20. |
| 5 | [#20](https://github.com/vm75/message-sync/issues/20) | Discord outbound text/media/replies/reactions/edits/deletes | #18, #19 | Complete | Implemented reusable per-channel managed webhooks with transient WhatsApp sender APP usernames/HMAC fallback, bidirectional transient media, reply mapping/fallback, reaction/edit/delete lifecycle, rate-limit retry handling, loop suppression, and privacy-safe tests/docs. Implementation commits `3046b4b`…`835e7ea`; gofmt-only cleanup `2b9c39c`…`3288a279`. Manual verification after `make fmt`: `go test ./...` passed, `go vet ./...` passed, `git diff --check` clean, `git diff --exit-code VERSION` clean, and `VERSION` remained `0.1.0`. Next ticket can rely on complete Discord outbound lifecycle; discovery/UI and threads/forums/polls remain staged. |
| 6 | [#21](https://github.com/vm75/message-sync/issues/21) | Discord discovery + admin web UI | #17, #18, #19 | Complete | Added authenticated Discord status/live channel discovery, safe per-alias managed-webhook readiness, deployment-only token guidance, and transport-neutral endpoint/mixed-sync-set UI. Implementation: 8652a01, aa8b30b, a286509. Verified `make fmt`, `git diff --check`, `git diff --exit-code`, `make test`, `make vet`, and unchanged `VERSION` (0.1.0). Next ticket can build on `/api/discord/status`, `/api/discord/channels`, and the mixed-transport endpoint UI. |
| 7 | [#22](https://github.com/vm75/message-sync/issues/22) | Threads/forums, mentions, polls, unsupported-format fallbacks | #20; #21 before UI exposure | Complete | Implemented deterministic thread/forum ingress flattening to configured parent aliases, privacy-safe mention text fallbacks, WhatsApp poll→Discord text rendering, sticker WebP fallback, and deterministic unsupported-format handling; privacy assertions verify Discord member/channel/guild metadata stays out of `sync.db`. Implementation through `64509c5`; manual verification: `make fmt`, `git diff --check`, `make test`, `make vet`, and `git diff --exit-code VERSION` passed; `VERSION` remains `0.1.0`. Next ticket can rely on documented format semantics without dynamic thread endpoint persistence. |
| 8 | [#23](https://github.com/vm75/message-sync/issues/23) | End-to-end hardening, docs, privacy review, tracker cleanup | #16–#22 | In progress | Started final mixed-transport hardening, sender-rendering verification, documentation/privacy review, and tracker cleanup. |
