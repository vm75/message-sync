...
| 4 | [#19](https://github.com/vm75/message-sync/issues/19) | Wire multiple adapters into application/router | #16, #18 | In progress | Implementation committed (`bd2d1c1`, formatting cleanup `7d81d84`): alias→transport adapter registry, one ordered WA/Discord ingress loop, mixed fan-out/restart tests, and docs. Verification blocker: GitHub Actions run 33128430051 failed before any steps (zero steps/logs), and the available local runtime has Go 1.23.2; required `make fmt`, `make test`, and `make vet` still need a successful manual run before completion. |
