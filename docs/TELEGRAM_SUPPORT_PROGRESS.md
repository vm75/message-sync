...
| 3 | [#27](https://github.com/vm75/message-sync/issues/27) | Wire Telegram into multi-adapter canonical router lifecycle | #25, #26 | Complete | Implementation through de625656; Telegram registered in the shared adapter registry/event loop with runtime config reloads; three-transport routing, safe credential/startup handling, remote-ID/canonical separation, and restart-safe partial retry covered; clean manual make fmt/test/vet validation passed at d7fbaec; VERSION unchanged; Telegram protocol outbound lifecycle remains #28. |
| 4 | [#28](https://github.com/vm75/message-sync/issues/28) | Telegram outbound text/media/replies/reactions/edits/deletes | #26, #27 | Complete | Implementation through 28830ea; transient text attribution + HMAC fallback, media/replies/reactions/edits/deletes, ordinary message_copies, privacy-safe bounded retries, and deterministic Telegram limitations covered; validated manual make fmt/test/vet passed; VERSION unchanged. |
| 5 | [#29](https://github.com/vm75/message-sync/issues/29) | Telegram chat discovery, runtime status, and admin Web UI | #25, #26, #27 | Complete | Implementation through 97af128; added bounded process-only observed group/supergroup discovery, authenticated Telegram status/chats APIs, safe privacy-mode readiness guidance, and three-transport admin UI endpoint/sync-set flow; API/UI/privacy tests added; clean manual make fmt/test/vet validation passed at 97af128; no schema/container/release changes; VERSION unchanged; next ticket is #30. |
| 6 | [#30](https://github.com/vm75/message-sync/issues/30) | Forum topics, polls, mentions, migrations, and format fallbacks | #28; #29 before related admin behavior | Complete | Implementation through ed00c64; parent-alias forum flattening, transactional group→supergroup addressing migration, deterministic text polls, privacy-safe mentions/service ignores, and media fallbacks documented/tested; validated manual make fmt/test/vet passed; VERSION unchanged. |
| 7 | [#31](https://github.com/vm75/message-sync/issues/31) | End-to-end hardening, docs, privacy review, tracker cleanup | #25–#30 | In progress | Started final three-transport hardening, documentation, privacy review, and tracker cleanup verification. |
