...
| 3 | [#27](https://github.com/vm75/message-sync/issues/27) | Wire Telegram into multi-adapter canonical router lifecycle | #25, #26 | In progress | Implementation through de625656: shared registry/event-loop/config-reload wiring plus three-transport and restart-safe retry tests; first manual run passed test/vet but revealed gofmt-only diffs, now committed; clean rerun pending; VERSION unchanged. |
