...
| State | [#35 Add a content-free delivery ledger](https://github.com/vm75/message-sync/issues/35) | Done | #32, #33; coordinate states with #34 | `0e5edf7` | `make fmt`; `GOCACHE=/tmp/message-sync-go-cache make test`; `GOCACHE=/tmp/message-sync-go-cache make vet`; `git diff --check`; `git diff --exit-code VERSION` — PASS | `ARCHITECTURE.md`, `.gitignore`, `internal/store/schema.sql`, `internal/store/store.go`, `internal/store/store_test.go` | Next: #36; no follow-up |
| Isolation | [#36 Add independent ordered delivery lanes](https://github.com/vm75/message-sync/issues/36) | Done | #34, #35 | `11eaf1b` | `make fmt`; `GOCACHE=/tmp/message-sync-go-cache make test`; `GOCACHE=/tmp/message-sync-go-cache make vet`; `GOCACHE=/tmp/message-sync-go-cache go test -race ./internal/router ./internal/delivery`; `git diff --check`; `git diff --exit-code VERSION` — PASS | `ARCHITECTURE.md`, `internal/app/app.go`, `internal/delivery`, `internal/router`, `docs/MUST_HAVE_SYNC_RELIABILITY_TRACKER.md` | Next: #37; no follow-up |
| Semantics | [#37 Add bounded retries and lifecycle ordering](https://github.com/vm75/message-sync/issues/37) | Done | #34-#36 | `e473d67` | `make fmt`; `make test`; `make vet`; `GOCACHE=/tmp/message-sync-go-cache go test -race ./internal/delivery ./internal/router`; `git diff --check`; `git diff --exit-code VERSION` — PASS | `ARCHITECTURE.md`, `internal/delivery/lanes.go`, `internal/router/router.go`, `internal/router/router_test.go`, `internal/transport/discord/e2e_test.go` | Bounded retries honor retry-after; lifecycle mutations are lane-ordered, coalesced, tombstone-aware, and replay-safe; privacy review PASS; Next: #38; no follow-up |
| Discord resilience | [#38 Make Discord managed webhooks self-healing](https://github.com/vm75/message-sync/issues/38) | Done | #34, #37 | `6a6c27e` | `go test ./internal/transport/discord -run 'TestManagedWebhook'`; `GOCACHE=/tmp/message-sync-go-cache go test -race ./internal/transport/discord`; `make fmt`; `git diff --check`; `GOCACHE=/tmp/message-sync-go-cache make test`; `GOCACHE=/tmp/message-sync-go-cache make vet`; `git diff --exit-code VERSION` — PASS | `ARCHITECTURE.md`, `docs/MUST_HAVE_SYNC_RELIABILITY_IMPLEMENTATION_PLAN.md`, `internal/transport/discord/webhook_manager.go`, `internal/transport/discord/webhook_manager_test.go`, `internal/transport/discord/e2e_test.go` | Per-channel webhook invalidation, single-flight list-or-create repair, exactly-one retry, and safe error-code distinction complete; privacy review PASS; no follow-up; Next: #39 |
| Recovery foundation | [#39 Add common accepted-event checkpoints and recovery coordination](https://github.com/vm75/message-sync/issues/39) | Done | #35, #37 | `6bbea23` | `make fmt`; `git diff --check`; `GOCACHE=/tmp/message-sync-go-cache make test`; `GOCACHE=/tmp/message-sync-go-cache make vet`; `GOCACHE=/tmp/message-sync-go-cache go test -race ./internal/recovery ./internal/router ./internal/delivery`; `git diff --exit-code VERSION` — PASS. Broader adapter race sweep remains limited by pre-existing unsynchronized test fakes/log buffers; issue-specific race scope passes. | `ARCHITECTURE.md`, `docs/MUST_HAVE_SYNC_RELIABILITY_IMPLEMENTATION_PLAN.md`, `internal/transport/transport.go`, `internal/store/schema.sql`, `internal/store/store.go`, `internal/recovery`, `internal/router`, `internal/app`, `internal/transport/{discord,telegram}`, `internal/*_test.go` | Accepted-event checkpoint and single-flight recovery foundation complete; privacy review PASS; no follow-up; Next: #40 |
| Telegram recovery | [#40 Persist and recover Telegram update offsets](https://github.com/vm75/message-sync/issues/40) | Done | `bccaba4` | `cad17a0` | `make fmt`; `git diff --check`; `GOCACHE=/tmp/message-sync-go-cache make test`; `GOCACHE=/tmp/message-sync-go-cache make vet`; `GOCACHE=/tmp/message-sync-go-cache go test -race ./internal/transport/telegram ./internal/recovery ./internal/router ./internal/delivery`; `git diff --exit-code VERSION` — PASS | `ARCHITECTURE.md`, `README.md`, `docs/MUST_HAVE_SYNC_RELIABILITY_IMPLEMENTATION_PLAN.md`, `internal/app/app.go`, `internal/transport/telegram/{adapter.go,adapter_test.go,admin_test.go}` | Persisted global Telegram cursor initializes Bot API polling at cursor+1; ignored updates use content-free coordinator no-ops; provider retained-window limitation documented; privacy review PASS; no migration/backward compatibility; Next: #41 |
| Discord recovery | [#41 Add bounded Discord channel history recovery](https://github.com/vm75/message-sync/issues/41) | In progress | #38, #39 | — | — | — | Starting at `42482d6820510bd4f489ec7332730ed6f2bdacdc`; implement bounded per-channel Discord history recovery through the common coordinator, with startup/reconnect signals and safe permission/rate-limit handling. Assumptions: configured endpoint aliases are recovery stream keys; Discord snowflakes are ordered cursors; deletes/reaction transitions unavailable from history remain documented provider limitations. |
